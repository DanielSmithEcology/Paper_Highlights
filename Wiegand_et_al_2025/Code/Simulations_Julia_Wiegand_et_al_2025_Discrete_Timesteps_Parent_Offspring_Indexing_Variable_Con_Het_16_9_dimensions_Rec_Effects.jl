...
# Parameters
Mx = 192/2  # Size of the torus (meters)
My = 108
...
birth_rates = fill(.2, S)  # Random birth rates for each species
...
C_effect_Rec = fill(.8,S)
H_effect_Rec = fill(.8,S)
...
function plot_aggregation_metric_vs_abundance_DF(spatial_locations, time_series, focal_species,r,Mx,My)
    ...
    println(length(merged_data.mean_HET_count))
    println(length(abundance_vector_others))
    merged_data.Scaled_mean_HET_count = merged_data.mean_HET_count./(c*abundance_vector_others)
...
# Function to calculate mortality rate recruits
function mortality_rate_recruits(population,i,xJ,yJ, r, C_effect, H_effect, CON, HET, Mx,My)
    focal = population[i, :]
    C_effect_i = C_effect[focal.species] 
    H_effect_i = H_effect[focal.species] 
    neighbors = population[[toroidal_distance(xJ, yJ, x, y, Mx,My) <= r for (x, y) in zip(population.x, population.y)] , :]
    distances = [toroidal_distance(xJ, yJ, x, y, Mx,My) for (x, y) in zip(neighbors.x, neighbors.y)]
    distance_CON = distances[neighbors.species .== focal.species]
    distance_HET = distances[neighbors.species .!= focal.species]
    CON_count = isempty(distance_CON) ? 0.0 : sum(1 ./ distance_CON)
    HET_count = isempty(distance_HET) ? 0.0 : sum(1 ./ distance_HET)
...
# Simulation function
function run_simulation(S,population, dispersal_centers, birth_rates, CON, HET, C_effect, H_effect,C_effect_Rec, H_effect_Rec, P_L, I, r, Disp_k, D_Change, Mx,My, tau, num_timesteps,S0)
    ...
    all_species = DataFrame(species = 1:S)
    ...
    for timestep in 1:num_timesteps
        ...
        for i in 1:size(population, 1)
            ...
            for _ in 1:num_offspring
                ...
                Rec_death =  mortality_rate_recruits(population,i,x, y, r, C_effect_Rec, H_effect_Rec, CON, HET, Mx,My)

                if rand() < exp(-Rec_death) 
        ...
        survivors = population[[rand() < S0*exp(-death_rate) for death_rate in death_rates], :]
        ...
        #    println("Timestep $timestep: Saving data...")
            ...
            for species in 1:S
                if !in(species, abundance.species)
                    push!(time_series, (timestep, species, 0))
                end
            end
...
num_timesteps = 500  # Number of timesteps to simulate
tau = 5
S0 = .9625
...
P_L = .95
Time_Series_95b, Spatial_Locations_95b, Population_95b, Centers_95b = run_simulation(S,population, dispersal_centers, birth_rates, CON, HET, C_effect, H_effect,C_effect_Rec, H_effect_Rec, P_L, I, r, Disp_k, D_Change, Mx, My, tau, num_timesteps,S0)

P_L = .5
Time_Series_5b, Spatial_Locations_5b, Population_5b = run_simulation(S,population, dispersal_centers, birth_rates, CON, HET, C_effect, H_effect,C_effect_Rec, H_effect_Rec, P_L, I, r, Disp_k, D_Change, Mx, My, tau, num_timesteps,S0)

P_L = .05
Time_Series_05b, Spatial_Locations_05b, Population_05b, Centers_05b = run_simulation(S,population, dispersal_centers, birth_rates, CON, HET, C_effect, H_effect, C_effect_Rec, H_effect_Rec,P_L, I, r, Disp_k, D_Change, Mx, My, tau, num_timesteps,S0)
...
# Plot the time series
function plot_time_series2(time_series)
    species_list = unique(time_series.species)
    plot()
    for species in species_list
        species_data = time_series[time_series.species .== species, :]
        plot!(species_data.timestep, species_data.abundance,legend=false)
    end
    xlabel!("Timestep")
    ylabel!("Abundance")
    title!("Abundance of Each Species Through Time")
...
plot_time_series2(Time_Series_05b)
plot_time_series2(Time_Series_95b)
