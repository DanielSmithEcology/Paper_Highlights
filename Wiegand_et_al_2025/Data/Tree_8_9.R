...
add_image_b <- function(plot, img_path, x, y, width, height) {
  img <- rasterGrob(png::readPNG(img_path), width = unit(width, "npc"), height = unit(height, "npc"))
  plot + annotation_custom(img, xmin = x - 0.025, xmax = x + 0.025, ymin = y - 0.025, ymax = y + 0.025)
}

# Will pake some pngs and combine them to gif (used for the X's for mortality)
...
Animate_All <- function(Spatial_Locations_Data,TimeSteps,Frames_Seeds,Tree_Images,Tree_WD,Seed_Image,X_Image_Path,alphabetical_vector,WD_Save,Scale_Factor,Mx,My){
  ...
  for(tt in 2:TimeSteps){
    ...
    for(dd in 1:(nrow(X_Y_Deaths))) {
      List_Plots[[dd+1]] <- add_image_b(List_Plots[[dd]],X_Image_Path,X_Y_Deaths$x[dd]/(Mx+10), X_Y_Deaths$y[dd]/(My+10),.35,.35) 
    }
    ...
    create_gif_from_plots(List_Plots,Name_animation_Deaths,2,800*3,900*3)
...
create_animation <- function(Adult_Positions, animation_data, Tree_Images,Tree_WD,Mx,My) {
  ...
  grid_data <- expand.grid(x = (0-5):(Mx+5), y = (0-5):(My+5))
  ...
  anim_plot <- ggplot(grid_data, aes(x, y)) +
    annotation_custom(texture, xmin = .5-5, xmax = Mx+5, ymin = .55-5, ymax = My+5) + # change these boundaries to make raster match 
  ...
  anim_plot_New <- animate(anim_plot, nframes = Frames_Seeds + 15, res = 75,fps = 10, height = 900*3, width = 1600*3/2, end_pause = 15, renderer = gifski_renderer())
...
Tree_WD <- "C:/Users/smith/OneDrive/Desktop/Videos and More/Paper_Highlights/Paper_Highlights/Wiegand_et_al_2025/Tree_Images_2"
...
Animate_All(Spatial_Locations_05,TimeSteps,Frames_Seeds,Tree_Images,Tree_WD,Seed_Image,X_Image_Path,alphabetical_vector,WD_Save,Scale_Factor,Mx,My)
